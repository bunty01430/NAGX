...
fn paste_from_clipboard(terminals: &TerminalSlots, ptys: &PtySlots, slot: usize) -> Result<(), String> {
    ...
    let bracketed = terminals.lock().map_err(|_| "terminal mutex poisoned".to_string())?[slot].bracketed_paste_enabled();
    ...
    let pty = ptys.lock().map_err(|_| "PTY mutex poisoned".to_string())?[slot].clone().ok_or_else(|| "Terminal session is not connected".to_string())?;
    pty.send(data)
}
...
fn spawn_terminal_connection(
    ...
    thread::spawn(move || {
        ...
        match runtime.block_on(ssh::connect_pty(host.clone(), 22, username.clone(), "password".to_string(), secret, String::new())) {
            Ok((pty, mut output_rx)) => {
                if let Ok(mut slots) = ptys.lock() { slots[slot] = Some(pty); }
                ...
                runtime.block_on(async move {
                    while let Some(bytes) = output_rx.recv().await {
                        ...
                        let _ = slint::invoke_from_event_loop(move || {
                            if let Some(window) = weak_for_output.upgrade() {
                                let styled = slint::StyledText::from_markdown(&markup).unwrap_or_else(|_| slint::StyledText::from_plain_text(&markup));
                                match slot {
                                    0 => { window.set_terminal_1_styled(styled); window.set_terminal_1_plain(plain.into()); window.set_terminal_1_cursor_x(i32::from(cursor_x)); window.set_terminal_1_cursor_y(i32::from(cursor_y)); window.set_terminal_1_cursor_visible(cursor_visible); window.set_terminal_1_mouse_reporting(mouse_reporting); }
                                    1 => { window.set_terminal_2_styled(styled); window.set_terminal_2_plain(plain.into()); window.set_terminal_2_cursor_x(i32::from(cursor_x)); window.set_terminal_2_cursor_y(i32::from(cursor_y)); window.set_terminal_2_cursor_visible(cursor_visible); window.set_terminal_2_mouse_reporting(mouse_reporting); }
                                    2 => { window.set_terminal_3_styled(styled); window.set_terminal_3_plain(plain.into()); window.set_terminal_3_cursor_x(i32::from(cursor_x)); window.set_terminal_3_cursor_y(i32::from(cursor_y)); window.set_terminal_3_cursor_visible(cursor_visible); window.set_terminal_3_mouse_reporting(mouse_reporting); }
                                    _ => {}
                                }
                            }
                        });
                    ...
                    let _ = slint::invoke_from_event_loop(move || {
                        if let Some(window) = weak_window.upgrade() {
                            ...
                            if slot == (window.get_active_terminal() as usize).saturating_sub(1) { window.set_status_text(format!("T{} PTY DISCONNECTED", slot + 1).into()); }
                        }
                    });
                });
            }
...
fn main() -> Result<(), slint::PlatformError> {
    ...
    window.on_connect_slot(move |slot, host, username, password| {
        ...
        spawn_terminal_connection(weak_for_connect.clone(), Arc::clone(&ptys_for_connect), Arc::clone(&terminals_for_connect), slot, host.to_string(), username.to_string(), password.to_string());
    });
    ...
    window.on_terminal_resize(move |slot, cols, rows, pixel_width, pixel_height| {
        ...
        { let mut terminals = terminals_for_resize.lock().expect("terminal mutex poisoned"); if terminals[slot].size() != (rows, cols) { terminals[slot].set_size(rows, cols); } }
    ...
    window.on_terminal_mouse(move |slot, button, kind, x, y, shift, alt, control| {
        let slot = (slot.clamp(1, 3) - 1) as usize;
        let bytes = terminals_for_mouse.lock().expect("terminal mutex poisoned")[slot].mouse_report(button.clamp(0, 5) as u8, kind.clamp(0, 3) as u8, x.clamp(1, 1000) as u16, y.clamp(1, 1000) as u16, shift, alt, control);
        if let Some(bytes) = bytes { if let Some(pty) = ptys_for_mouse.lock().expect("PTY mutex poisoned")[slot].clone() { let _ = pty.send(bytes); } }
    });
